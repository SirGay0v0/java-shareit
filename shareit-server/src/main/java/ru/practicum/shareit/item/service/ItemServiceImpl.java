...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemForOwnerDto> getAllById(Long userId, int from, int size) {
        ...
        for (ItemForOwnerDto itemForOwnerDto : list) {
            List<Booking> bookList = bookingStorage.findByItemId(itemForOwnerDto.getId());
            if (bookList.isEmpty()) {
                return list;
            } else {
                NextBookingDto next = getNext(itemForOwnerDto.getId());
                LastBookingDto last = getLast(itemForOwnerDto.getId());

                if (last != null && next != null &&
                        last.getId().equals(next.getId()) && last.getBookerId().equals(next.getBookerId())) {
                    itemForOwnerDto.setNextBooking(next);
                    resultList.add(itemForOwnerDto);
                } else {
                    itemForOwnerDto.setNextBooking(next);
            ...
            List<Comment> commentsList = commentsStorage.findByItemId(itemForOwnerDto.getId());
            List<RequestCommentDto> requestComments = commentsList.stream()
                    .map(comment -> mapper.map(comment, RequestCommentDto.class))
                    .collect(Collectors.toList());
            if (commentsList.isEmpty()) {
                itemForOwnerDto.setComments(Collections.emptyList());
            } else {
                itemForOwnerDto.setComments(requestComments);
            }
            resultList.add(itemForOwnerDto);
        }
        return resultList;
